...
import qr.simulate_federated_qr_orthonormalisation as fed
import qr.shared_functions as sh
import qr.convenience as cv
...
if __name__ == '__main__':
    ...
    pd.DataFrame(diabetes_X).to_csv('../data/diabetes/diabetes.tsv', header=False, index=False)
    pd.DataFrame(diabetes_y).to_csv('../data/diabetes/diabetes_lab.tsv', header=False, index=False)
    ...
    with open('../results/diabetes.txt', 'w') as handle:
    ...
    # Fish market
    fish = pd.read_csv('../data/fish/Fish.csv')
    fish = fish.values
    weight = fish[:,1]
    ...
    print(rsquared)
    print(pval)
    with open('../results/fish.txt', 'w') as handle:
        handle.write(cv.collapse_array_to_string(x,'Coefficients:'))
        handle.write('r-squared:\t'+ str(rsquared)+'\n')
        handle.write(cv.collapse_array_to_string(pval,'p-values:'))
    ...
    ### WHO
    print('WHO')
    who = pd.read_csv('../data/who/who.csv')
    who = who.interpolate()
    who.to_csv('../data/who/who2.csv', index=False)
    ...
    with open('../results/who.txt', 'w') as handle:
        handle.write(cv.collapse_array_to_string(x,'Coefficients:' ))
        handle.write('r-squared:\t'+ str(rsquared)+'\n')
        handle.write(cv.collapse_array_to_string(pval,'p-values:'))
